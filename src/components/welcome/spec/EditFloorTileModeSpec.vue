<template>
  <spec-frame
    property="editFloorTileMode"
    label-str="フロアタイル変更モード"
    lastVersion="1.0.0b1"
    :isSpecFixed="true"
  >
    <!----------------------
    ...
    <spec-item class-str="spec" label-str="仕様">
      <li>
        このモードの最中はマップオブジェクトを全て非表示の状態にする
      </li>
      <li>
        画像とマス数だけを指定してマップに画像の被せ物ができるイメージ
      </li>
      <li>
        フロアタイルはフロアタイル変更モード中にしか編集できない
      </li>
      <li>
        フロアタイルはあらゆるマップオブジェクトより下に表示され、マップより上に表示される
      </li>
    </spec-item>
    <!----------------------
     ! ユーザ要望
    ...
    <spec-item class-str="unSupport" label-str="実装しない機能">
      <li>
        どどんとふではフロアタイルがマップからはみ出て配置されても表示され、その上で操作も可能だが、Quoridornではテーブルからはみ出たら切り捨てられるものとする。
      </li>
    </spec-item>
    <!----------------------
     ! 関連項目
