  ...
  methods: {
    ...
    leftUp() {
      ...
      if (this.rollObj.isRolling) {
        ...
        ].list.filter(obj => obj.key === this.rollObj.key)[0];
              ...
              reader.onload = function(event) {
                if (isThumbnail) {
                  ...
                  image.onload = function() {
                    ...
                    // 大きい場合は、比率を保ったまま縮小する
                    if (
                      useSize.w > thumbnailSize.w ||
                      useSize.h > thumbnailSize.h
                    ) {
                      const scale = Math.min(
                        thumbnailSize.w / useSize.w,
                        thumbnailSize.h / useSize.h
                      );
      ...
      handler(mouseLocate) {
        ...
        if (this.isDraggingRight) {
          ...
          let angleDiff = this.arrangeAngle(
            angle - this.anglevolatile.dragStart
          );
          this.setProperty({
