...
export default {
  methods: {
    ...
    calcCoordinate(screenX, screenY, oldAngle) {
      ...
      const distance =
        Math.sqrt(Math.pow(loc.x, 2) + Math.pow(loc.y, 2)) * zoom;
      // マップ回転前の角度を求める
      ...
      */
      // window.console.qLog(`screen(${this.f(screenX)}, ${this.f(screenY)}), angle:${this.f(angle)}, distance:${this.f(distance)} plane(${this.f(planeLocate.x)}, ${this.f(planeLocate.y)})`)
      return {
        angle: angle, // 角度
    ...
    calcAddress(screenX, screenY, oldAngle, offsetX = 0, offsetY = 0) {
      ...
      let planeLocateCanvas = this.calcCoordinate(screenX, screenY, oldAngle)
        .planeLocateCanvas;
