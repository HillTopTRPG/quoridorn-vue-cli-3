...
export default {
  ...
  methods: {
    ...
    changeFontColorType(key, value) {
      const targetCharacter = this.characters.filter(
    ...
    getMembers() {
      return function() {
        return this.members.filter(
          member => member.name === this.currentPlayerKey
        );
      };
    },
    getMapObject(state) {
      return function(kind, place) {
        return state.public[kind].list.filter(
          target =>
            target.owner === this.currentPlayerKey && target.place === place
        );
      };
    }
  })
